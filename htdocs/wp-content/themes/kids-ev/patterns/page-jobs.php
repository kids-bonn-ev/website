<?php
/**
 * Title: Seite: Jobs
 * Slug: kids/page-jobs
 * Categories: kids
 * Description: Hero, Benefits, pädagogischer Ansatz, offene Stellen und Bewerbungsablauf.
 * Viewport Width: 1200
 * Inserter: yes
 */
?>
<!-- wp:kids/hero {"deko":[{"name":"kreis","size":70,"top":"20%","right":"10%","rotate":0,"opacity":1,"hide_mobile":true},{"name":"blume01","size":90,"bottom":"10%","left":"8%","rotate":-10,"opacity":1,"hide_mobile":true},{"name":"welle","size":130,"top":"12%","left":"12%","rotate":0,"opacity":0.7,"hide_mobile":true}]} -->
<!-- wp:paragraph {"className":"is-style-eyebrow"} -->
<p class="is-style-eyebrow">Jobs @ KIDS</p>
<!-- /wp:paragraph -->

<!-- wp:heading {"level":1} -->
<h1 class="wp-block-heading">Pädagogik mit Rückenwind.</h1>
<!-- /wp:heading -->

<!-- wp:paragraph {"className":"is-style-lede"} -->
<p class="is-style-lede">Arbeiten in einer Bonner Elterninitiative: kleine Gruppen, starke Eltern-Community, und der Raum, wirklich pädagogisch zu arbeiten.</p>
<!-- /wp:paragraph -->

<!-- wp:kids/button-row {"align_row":"center"} -->
<!-- wp:kids/button {"label":"Zu den Stellen","href":"#stellen"} /-->
<!-- /wp:kids/button-row -->
<!-- /wp:kids/hero -->

<!-- wp:kids/section -->
<!-- wp:kids/section-head {"eyebrow":"Warum KIDS?","heading":"Was bei uns konkret anders ist."} /-->

<!-- wp:group {"className":"benefits","layout":{"type":"default"}} -->
<div class="wp-block-group benefits"><!-- wp:kids/card {"variant":"accent-top"} -->
<!-- wp:heading {"level":3} -->
<h3 class="wp-block-heading">Mitgestalten, nicht abarbeiten.</h3>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>Konzept, Tagesablauf, Material - du prägst das mit. Keine hundertste Trägervorgabe.</p>
<!-- /wp:paragraph -->
<!-- /wp:kids/card -->

<!-- wp:kids/card {"variant":"accent-top"} -->
<!-- wp:heading {"level":3} -->
<h3 class="wp-block-heading">Pädagogik bleibt Pädagogik.</h3>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>Eltern tragen den Rahmen: Einkauf, Gebäude, Feste, Finanzen. Du bist Expert:in für die pädagogische Arbeit. Klare Rollen, klare Köpfe.</p>
<!-- /wp:paragraph -->
<!-- /wp:kids/card -->

<!-- wp:kids/card {"variant":"accent-top"} -->
<!-- wp:heading {"level":3} -->
<h3 class="wp-block-heading">TvöD und Weihnachtsgeld.</h3>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>Faire, transparente Vergütung nach Tarif. Dazu Weihnachtsgeld.</p>
<!-- /wp:paragraph -->
<!-- /wp:kids/card -->

<!-- wp:kids/card {"variant":"accent-top"} -->
<!-- wp:heading {"level":3} -->
<h3 class="wp-block-heading">Wachsen gehört zur Arbeit.</h3>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>Supervision, Fortbildungen, wöchentliche Teambesprechungen, betriebliche Weiterbildung.</p>
<!-- /wp:paragraph -->
<!-- /wp:kids/card -->

<!-- wp:kids/card {"variant":"accent-top"} -->
<!-- wp:heading {"level":3} -->
<h3 class="wp-block-heading">Neubau mit 1.700 m² Garten.</h3>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>Turnhalle mit Kletterwand, Werkstatt, Atelier, eigene Küche. Pädagogik im Freien, jeden Tag.</p>
<!-- /wp:paragraph -->
<!-- /wp:kids/card -->

<!-- wp:kids/card {"variant":"accent-top"} -->
<!-- wp:heading {"level":3} -->
<h3 class="wp-block-heading">Eltern auf Augenhöhe.</h3>
<!-- /wp:heading -->

<!-- wp:paragraph -->
<p>Keine Kunden. Partner:innen, die Verantwortung mittragen.</p>
<!-- /wp:paragraph -->
<!-- /wp:kids/card --></div>
<!-- /wp:group -->
<!-- /wp:kids/section -->

<!-- wp:kids/section {"background":"cream"} -->
<!-- wp:columns -->
<div class="wp-block-columns"><!-- wp:column {"width":"40%"} -->
<div class="wp-block-column" style="flex-basis:40%"><!-- wp:paragraph {"className":"is-style-eyebrow"} -->
<p class="is-style-eyebrow">Unser Ansatz</p>
<!-- /wp:paragraph -->

<!-- wp:heading {"level":2} -->
<h2 class="wp-block-heading">Das Kind<br>als Akteur.</h2>
<!-- /wp:heading --></div>
<!-- /wp:column -->

<!-- wp:column {"width":"60%"} -->
<div class="wp-block-column" style="flex-basis:60%"><!-- wp:paragraph -->
<p>Wir sehen das Kind als <strong>selbstbestimmten Akteur seiner Entwicklung</strong> und gestalten den Alltag situationsorientiert.</p>
<!-- /wp:paragraph -->

<!-- wp:paragraph -->
<p>Eine offene und freundliche Atmosphäre liegt uns am Herzen. Wir leben den Kita-Alltag so nachhaltig wie möglich. Und wir freuen uns, gemeinsam mit dem Team unsere neue Kita in der Limpericher Straße weiter zu entwickeln und wachsen zu lassen.</p>
<!-- /wp:paragraph --></div>
<!-- /wp:column --></div>
<!-- /wp:columns -->
<!-- /wp:kids/section -->

<!-- wp:kids/section {"anchor":"stellen"} -->
<!-- wp:kids/section-head {"eyebrow":"Offene Stellen","heading":"Komm ins Team."} /-->

<!-- wp:group {"className":"stellen-list","layout":{"type":"default"}} -->
<div class="wp-block-group stellen-list"><!-- wp:kids/card {"anchor":"paedagogik"} -->
<!-- wp:heading {"level":3} -->
<h3 class="wp-block-heading">Pädagogische Mitarbeiter:innen (m/w/d)</h3>
<!-- /wp:heading -->

<!-- wp:kids/job-meta {"items":[{"text":"Eintritt: <strong>ab sofort</strong>"},{"text":"Laufend offen"},{"text":"Teil- oder Vollzeit"}]} /-->

<!-- wp:paragraph -->
<p>Wir suchen <strong>laufend</strong> pädagogische Fachkräfte für unsere drei Gruppen Mond, Sonne und Zwerge. Die Stelle ist evergreen. Auch wenn du externe Ausschreibungen mit abgelaufenen Fristen findest, nehmen wir Bewerbungen jederzeit entgegen.</p>
<!-- /wp:paragraph -->

<!-- wp:columns {"className":"stelle-grid"} -->
<div class="wp-block-columns stelle-grid"><!-- wp:column -->
<div class="wp-block-column"><!-- wp:heading {"level":4} -->
<h4 class="wp-block-heading">Was dich erwartet</h4>
<!-- /wp:heading -->

<!-- wp:list {"className":"is-style-kids-bullets"} -->
<ul class="wp-block-list is-style-kids-bullets"><!-- wp:list-item -->
<li>Neubau mit 1.700&nbsp;m² Garten und vielfältigen Möglichkeiten</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Raum, dich aktiv und kreativ einzubringen</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Enge Zusammenarbeit mit einer engagierten Elternschaft</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Supervision, Fortbildungen, wöchentliche Teambesprechungen</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Vergütung nach TvöD</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Weihnachtsgeld</li>
<!-- /wp:list-item --></ul>
<!-- /wp:list --></div>
<!-- /wp:column -->

<!-- wp:column -->
<div class="wp-block-column"><!-- wp:heading {"level":4} -->
<h4 class="wp-block-heading">Was du mitbringst</h4>
<!-- /wp:heading -->

<!-- wp:list {"className":"is-style-kids-bullets"} -->
<ul class="wp-block-list is-style-kids-bullets"><!-- wp:list-item -->
<li>Abgeschlossene Ausbildung für pädagogische Arbeit in einer Kita</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Wertschätzende Grundhaltung</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Motivation und Engagement</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Teamfähigkeit</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Zuverlässigkeit</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Gestaltungswillen</li>
<!-- /wp:list-item --></ul>
<!-- /wp:list --></div>
<!-- /wp:column --></div>
<!-- /wp:columns -->

<!-- wp:group {"className":"stelle-cta","layout":{"type":"default"}} -->
<div class="wp-block-group stelle-cta"><!-- wp:kids/button {"label":"Jetzt bewerben","href":"mailto:kita@kids-bonn.de"} /--></div>
<!-- /wp:group -->
<!-- /wp:kids/card -->

<!-- wp:kids/card {"anchor":"kueche"} -->
<!-- wp:heading {"level":3} -->
<h3 class="wp-block-heading">Köchin / Koch (m/w/d)</h3>
<!-- /wp:heading -->

<!-- wp:kids/job-meta {"items":[{"text":"Eintritt: <strong>ab sofort</strong>"},{"text":"Elternzeitvertretung"},{"text":"30–40&nbsp;Std./Woche"}]} /-->

<!-- wp:paragraph -->
<p>Für die Elternzeitvertretung suchen wir eine Köchin / einen Koch, die unsere rund 55 Kinder und das Team täglich frisch versorgt - vegetarisch und in Bio-Qualität.</p>
<!-- /wp:paragraph -->

<!-- wp:columns {"className":"stelle-grid"} -->
<div class="wp-block-columns stelle-grid"><!-- wp:column -->
<div class="wp-block-column"><!-- wp:heading {"level":4} -->
<h4 class="wp-block-heading">Deine Aufgaben bei uns</h4>
<!-- /wp:heading -->

<!-- wp:list {"className":"is-style-kids-bullets"} -->
<ul class="wp-block-list is-style-kids-bullets"><!-- wp:list-item -->
<li>Tägliches frisches Mittagessen für rund 55 Kinder und das Team</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Vegetarische Vollwertküche in Bio-Qualität</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Eigenständige Planung abwechslungsreicher, kindgerechter Speisepläne</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Bestellung und Organisation der Lebensmittel</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Einhaltung der Hygienevorschriften (HACCP)</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Enge Abstimmung mit unserer Kita-Leitung</li>
<!-- /wp:list-item --></ul>
<!-- /wp:list --></div>
<!-- /wp:column -->

<!-- wp:column -->
<div class="wp-block-column"><!-- wp:heading {"level":4} -->
<h4 class="wp-block-heading">Wir bieten</h4>
<!-- /wp:heading -->

<!-- wp:list {"className":"is-style-kids-bullets"} -->
<ul class="wp-block-list is-style-kids-bullets"><!-- wp:list-item -->
<li>Raum, Speisepläne und Abläufe aktiv und kreativ mitzugestalten</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Eine moderne, gut ausgestattete Kita-Küche</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Kostenloses Mittagessen</li>
<!-- /wp:list-item --></ul>
<!-- /wp:list --></div>
<!-- /wp:column --></div>
<!-- /wp:columns -->

<!-- wp:columns {"className":"stelle-grid"} -->
<div class="wp-block-columns stelle-grid"><!-- wp:column -->
<div class="wp-block-column"><!-- wp:heading {"level":4} -->
<h4 class="wp-block-heading">Wir wünschen uns</h4>
<!-- /wp:heading -->

<!-- wp:list {"className":"is-style-kids-bullets"} -->
<ul class="wp-block-list is-style-kids-bullets"><!-- wp:list-item -->
<li>Freude am Kochen für Kinder und an gesunder, vegetarischer Küche</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Interesse an Nachhaltigkeit und Bio-Lebensmitteln</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Selbstständige und strukturierte Arbeitsweise</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Teamfähigkeit und Offenheit für Austausch</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Kreativität bei der Speiseplanung</li>
<!-- /wp:list-item --></ul>
<!-- /wp:list --></div>
<!-- /wp:column -->

<!-- wp:column -->
<div class="wp-block-column"><!-- wp:heading {"level":4} -->
<h4 class="wp-block-heading">Wir erwarten</h4>
<!-- /wp:heading -->

<!-- wp:list {"className":"is-style-kids-bullets"} -->
<ul class="wp-block-list is-style-kids-bullets"><!-- wp:list-item -->
<li>Wertschätzende Grundhaltung gegenüber Kindern und Team</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Zuverlässigkeit und Verantwortungsbewusstsein</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Hygienisches und sorgfältiges Arbeiten</li>
<!-- /wp:list-item -->

<!-- wp:list-item -->
<li>Gestaltungswillen und Freude an Weiterentwicklung</li>
<!-- /wp:list-item --></ul>
<!-- /wp:list --></div>
<!-- /wp:column --></div>
<!-- /wp:columns -->

<!-- wp:group {"className":"stelle-cta","layout":{"type":"default"}} -->
<div class="wp-block-group stelle-cta"><!-- wp:kids/button {"label":"Jetzt bewerben","href":"mailto:vorstand@kids-bonn.de"} /-->

<!-- wp:paragraph {"className":"stelle-hint"} -->
<p class="stelle-hint">Gerne mit beispielhaftem Wochen-Essensplan.</p>
<!-- /wp:paragraph --></div>
<!-- /wp:group -->
<!-- /wp:kids/card --></div>
<!-- /wp:group -->
<!-- /wp:kids/section -->

<!-- wp:kids/section {"background":"sand"} -->
<!-- wp:kids/section-head {"eyebrow":"Bewerbung","heading":"Kein Ritual. Kein Papierberg."} /-->

<!-- wp:columns -->
<div class="wp-block-columns"><!-- wp:column {"width":"55%"} -->
<div class="wp-block-column" style="flex-basis:55%"><!-- wp:kids/steps {"style":"circle-badge"} -->
<!-- wp:kids/step {"title":"Schreib uns eine Mail."} -->
<!-- wp:paragraph -->
<p>Ein kurzer Lebenslauf reicht. Wir brauchen keine Mappe.</p>
<!-- /wp:paragraph -->
<!-- /wp:kids/step -->

<!-- wp:kids/step {"title":"Wir melden uns innerhalb weniger Tage."} -->
<!-- wp:paragraph -->
<p>Telefonisch oder per Mail.</p>
<!-- /wp:paragraph -->
<!-- /wp:kids/step -->

<!-- wp:kids/step {"title":"Wir lernen uns kennen."} -->
<!-- wp:paragraph -->
<p>Erst im Gespräch, dann bei einem Hospitationstag im Haus.</p>
<!-- /wp:paragraph -->
<!-- /wp:kids/step -->

<!-- wp:kids/step {"title":"Entscheidung."} -->
<!-- wp:paragraph -->
<p>Für beide Seiten, in Ruhe.</p>
<!-- /wp:paragraph -->
<!-- /wp:kids/step -->
<!-- /wp:kids/steps --></div>
<!-- /wp:column -->

<!-- wp:column {"width":"45%"} -->
<div class="wp-block-column" style="flex-basis:45%"><!-- wp:kids/card -->
<!-- wp:heading {"level":3} -->
<h3 class="wp-block-heading">Deine Ansprechperson</h3>
<!-- /wp:heading -->

<!-- wp:paragraph {"className":"apply-name"} -->
<p class="apply-name">Lena&nbsp;Steib</p>
<!-- /wp:paragraph -->

<!-- wp:paragraph {"className":"apply-contacts"} -->
<p class="apply-contacts"><a href="mailto:kita@kids-bonn.de">kita@kids-bonn.de</a><br><a href="tel:+492284787950">0228&nbsp;478795</a></p>
<!-- /wp:paragraph -->

<!-- wp:paragraph {"className":"is-style-hand apply-hand"} -->
<p class="is-style-hand apply-hand">Lieber erst ein Kaffee? Ruf einfach an.</p>
<!-- /wp:paragraph -->
<!-- /wp:kids/card --></div>
<!-- /wp:column --></div>
<!-- /wp:columns -->
<!-- /wp:kids/section -->
